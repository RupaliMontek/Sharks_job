...
class Candidate_profile_login extends CI_Controller
{
	...
	public function check_user_login_check_candidate()
	{
        ...
        if($this->form_validation->run()==true)
        {
	        ...
	        $datas = $this->M_Candidate_profile_login->login_check_candidate($mailid,$pass);
          	if(!empty($datas)) 
	        {	$name = $datas[0]['name'];
	...
	public function check_user_login_by_otp_candidate()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($otp == $session_otp && $email == $session_email) {
                // OTP is valid, proceed with login
                $datas = $this->M_Candidate_profile_login->get_user_by_email($email);
                ...
                $this->session->set_flashdata('error', 'Invalid OTP or email');
                redirect('Candidate_profile_login');
            }
        } else {
            // Validation failed
...
public function check_phone_no_exists_registration()
 {
		...
		$result=$this->M_Candidate_profile->check_if_phone_exists($phone);
        if($result>=1)
		{
		    echo "false";
		}
...
public function check_password_exist(){
		...
		$result=$this->M_Candidate_profile->check_if_password_exists($enc_pass);
        if($result>=1)
		{
		    echo "true";
		}
	...
	{	      
        ...
        $login_id = $this->M_Candidate_profile->get_loginid();
        ...
        $url  = '<a target="_blank" href="' . base_url('Candidate_profile_login/activate_acc_home/' . $code) . '">Verify Email</a>';
...
/*	print_r($this->db->last_query());die();*/
    		...
    		if ($num_res === 1)
    		{
    			if ($res)
    			{
    				$url  = '<a target="_blank" href="' . base_url('Candidate_profile_login/change_password_home/' . $code) . '">Change Password</a>';
	...
	public function change_password_home($code)
	{
		...
		$result = $this->M_Candidate_profile_login->check_code_for_reset_password($code);
	 
	 
 		$data['record1'] = $this->M_Candidate_profile_login->getemail($code);
 	
		if ($result === '1')
		{
		     redirect("Candidate_profile_login/new_pass1/$code");
...
public function send_confirmation_mail($message,$email_id,$subject_data)
	...
	public function update_password()
	{
		...
		$res=$this->M_Candidate_profile_login->does_code_match($data['code'],$candidate_email);
		...
		$result=$this->M_Candidate_profile_login->update_user($data,$candidate_email);
//  	print_r($result);
			if ($result)
			{
